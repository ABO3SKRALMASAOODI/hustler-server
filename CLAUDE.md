...
- **LLM provider = DeepSeek V4 Pro (default, since Jul 26 2026).** The whole LLM stack (agent tool-calling, vision, concierge) is OpenAI-compatible. You set **only `OPENAI_API_KEY`** (a DeepSeek key) on both the backend web service and the worker — the defaults already select `OPENAI_BASE_URL=https://api.deepseek.com`, `AGENT_MODEL=deepseek-v4-pro`, `VISION_MODEL=deepseek-v4-pro`. To use a different provider set `OPENAI_BASE_URL`/`AGENT_MODEL`/`VISION_MODEL` (for the previous stack: `https://api.x.ai/v1` + `grok-4.5` + prices `2.0`/`6.0`).
  - **`deepseek-v4-pro`, never `deepseek-v4-flash`, for `VISION_MODEL`.** Flash is text-only — it ignores or rejects `image_url` parts, which would blind `look_at` and every contact-sheet read without raising anything you'd notice. Pro is the only V4 tier that accepts images, so agent and vision share it.
  - **Credit economics — read this.** V4 Pro is **$1.74 in / $3.48 out per 1M tokens** (vs Grok 4.5's $2/$6), so a turn costs ~13% less in and ~42% less out. `LLM_PRICE_IN_PER_M`/`LLM_PRICE_OUT_PER_M` default to `1.74`/`3.48` — **if you change `AGENT_MODEL`, change these to match** (they live in `worker/config.py`, `worker/db.py`, `backend/routes/admin_video.py`).
