...
- **ROUND 67 — Luna, the agent's own eyes, and the day two customers got nothing (Jul 31 2026).** Two real accounts failed the same afternoon and the owner traced five product-taste defects on top; everything below shipped together.
  - **THE AGENT RUNS ON OPENAI GPT-5.6 LUNA (owner's decision).** `OPENAI_BASE_URL` defaults to `https://api.openai.com/v1`, `AGENT_MODEL`/`VISION_MODEL`/`CONCIERGE_MODEL` to **`gpt-5.6-luna`** — the EXACT id matters, the bare `gpt-5.6` alias routes to Sol (a different, pricier model). List price from OpenAI's own model page: **$0.20 in / $0.02 cached / $1.20 out per 1M** (a third-party blog quotes $1/$6 — wrong), in `model_prices.MODEL_PRICES` on both mirrors; the `LLM_PRICE_*` fallbacks track it. Reasoning tokens FOLDED (`reasoning_separate: False`) — OpenAI's completion_tokens already contains them. Vision shares the provider and key (Luna takes images), so ONE `OPENAI_API_KEY` on both services lights up agent + vision + concierge; image gen stays on its own xAI provider. **Set an OpenAI `OPENAI_API_KEY` on the backend AND the worker before this deploy goes live, or every turn 401s.** DIALECT: OpenAI's reasoning family rejects the classic `max_tokens` (wants `max_completion_tokens`) and any non-default `temperature` — `llm.completion_kwargs`/`adapt_completion_kwargs` send the classic spelling, adapt on the narrow unknown-parameter 400 and latch per model (same policy as `reasoning_effort`), used by the agent loop, ask_text, ask_vision, the honesty regen and (inline) the backend concierge. `AGENT_REASONING_EFFORT=low` still applies from iteration 2.
  - **look_at IS THE AGENT'S OWN EYES NOW.** The vision-sub-agent middleman is gone when the agent model reads images: `look_at(times=[...])` (1-8 exact seconds; start/end still sample a range) captures the frames, assembles multiples into ONE timestamp-labeled sheet (`sheets.build_timestamp_sheet`, 480px tiles), and the loop injects it as image parts in a user message right after the tool results — the editor sees the footage and judges it directly. Same for `look_at_asset` (including kind='render' self-checks). Guard rails: `ctx.pending_images` clears per step; `_messages_for_record` keeps base64 out of llm_calls; a provider that 400s on the image part latches `llm.mark_agent_blind` and the tools fall back to the separate `VISION_*` provider (`AGENT_MULTIMODAL=0` forces that). Prompt: use it when the decision NEEDS the pixels, batch moments into one call, never look reflexively.
  - **DEEPGRAM CANNOT HEAR ARABIC AND SAID NOTHING (abrayen626, projects 295/296).** nova-3's `detect_language` picks the nearest member of its OWN short language list, so 130s of Arabic came back "Portuguese"/"Turkish" with **0 words as a 2xx** — and the whisper fallback only ran on FAILURES, so the agent told the user his video had no speech and refused captions/silence-cuts twice. `transcribe._too_sparse` now treats (near-)zero words on real-length audio as the failure it is and re-runs local whisper (~100 languages), keeping whichever engine heard more and recording a warning. Thresholds conservative (0 words at ≥3s; <3 words/min past 20s) — a genuinely silent video flies through whisper's VAD in seconds.
  - **THE BRAND KEYTERM HALLUCINATED ITSELF AS A CUSTOMER'S TRANSCRIPT (ayetoluabeebat, index 201).** `WHISPER_HOTWORDS` defaulted to "Valmera, valmera.io" and was fed to Deepgram's keyterm biasing on EVERY customer's audio: her 27s clip of a woman speaking transcribed as language "Indonesian" with exactly one word — **"Valmera."** — which captions then burned onto her video. Default is now **empty**; brand hotwords are for deployments that transcribe our own demo footage, set explicitly. Both transcription changes → **PIPELINE_VERSION 8**, so the broken zero-word indexes rebuild on next open.
  - **THE ERASE/REPAINT PASS MOVED TO THE EXECUTOR (`clean` job kind).** Her second failure: "I lost my connection" — job 1557 died mid-turn ("Worker died and retries are exhausted") minutes after her `erase_burned_text` ran the inpaint pass on the dispatcher: a full decode+re-encode of the ORIGINAL inside an agent turn, the heaviest member of the OOM class (rounds 61/62/64/65 moved every other original-decoder off that box; this was the last). `inpaint.run_clean_job` (capture-shaped, `REMOTE_TIMEOUT_CLEAN_S` 600) stages, repaints, uploads AND measures the before/after ink itself, so the dispatcher never touches the original; bounds are checked from the INDEX; remote failure is an honest refusal, never a local retry. No-executor deployments keep the local path unchanged.
  - **SOUND EFFECTS ARE STRICTLY OPT-IN, EVERYWHERE THE AGENT READS.** The taste audit had the rule since round 55 and the agent still carpeted videos, because the prompt's own recipes said "a riser INTO the biggest one" / "light sfx" and bullet 49 opened with "SOUND EFFECTS are how short-form holds attention". The recipes are cleaned, the bullet now OPENS with the prohibition, and `add_sfx`/`sound_design_pass` tool descriptions carry it too: 'make it viral/punchy' is NOT a request for sfx — offer them in the reply.
  - **CAPTIONS: bottom is the law, spotlight is the exception.** Every multi-word preset anchored "middle" — a text block across the speaker's face. All multi-word presets now default **bottom**; the new **`spotlight`** preset is the modern single-word look (ONE glowing uppercase word dead centre, karaoke mode with 1-word groups on the stack path so the glow under-layers render) and is the only preset allowed mid-frame. Prompt: pick the family by the video (fast/hype → spotlight; talking-head/podcast → dynamic at the bottom; luxury → luxe/editorial); taste.critique flags multi-word captions written at position middle. `test_round67.py` pins the preset table.
  - **ZOOMS ARE GENTLE BY DEFAULT.** `add_zoom` default strength 0.25→**0.15**, `punch_in_on_emphasis` 4×0.35→**3×0.14**, prompt teaches 8-18% eased pushes with at most ONE hard punch on the single peak, and taste.critique flags ≥2 hard 30%+ punches unless the user asked. Plus PLAN-FIRST: the first step of any real edit names the format and settles the whole edit (cuts, caption family+placement, the 2-3 zoom moments, music, open/close) before the first write.
  - **VERSION-ROLLBACK CHATTER IS GONE.** `_branch_edl` no longer writes "went back to edit state vX" into the chat (the edls table records lineage), the studio's "Editing from an earlier state" banner is removed, and stored revert activity rows from old sessions are filtered out of the render. **The pin also survives navigation now**: rolling back, visiting /admin and returning lands on the version the user chose (sessionStorage per project, restored in openProject, cleared wherever the code unpins).
  - Needs the usual **`gcloud run deploy valmera-executor --source worker/ --region us-central1`** from a clean `main` — the executor gains the `clean` job kind and PIPELINE_VERSION 8, and a stale executor would fail every remote erase with "unsupported job type".

- **ROUND 68 — Luna takes tools only with effort 'none', the matte stops keying on the walker, and the same video uploaded twice (Jul 31 2026).** Six defects from one afternoon in the test account (projects 298/300), each verified against prod rows before fixing:
  - **THE AGENT WAS DEAD ON LUNA + TOOLS.** OpenAI rejects function tools on /v1/chat/completions for gpt-5.6-luna unless `reasoning_effort` is EXPLICITLY `'none'` — an ABSENT field means default reasoning, which 400s ("Function tools with reasoning_effort are not supported… use /v1/responses or set reasoning_effort to 'none'"). Job 1626 died in 4s on the first Luna agent turn: the round-58 matcher only knew "**is** not supported" (OpenAI says "**are**"), and the strip-the-field latch could never fix an error the absent field causes. Now `llm.looks_like_tools_reasoning_conflict` latches `llm.tools_need_effort_none(model)` and every tools call to that model — agent loop AND the honesty regen via `create_with_dialect` — carries `reasoning_effort='none'`; `_BAD_PARAM_MARKERS` says "not supported". One 400 per process, by design. Reasoning WITH tools needs the /v1/responses API — a real future migration, deliberately not attempted inline.
  - **matte v9 (VERSION 9): per-pixel confidence, zero per-frame context.** The round-67 screenshots showed the armchair occluding the title exactly while the walker was NEAR it (its blob merged with his and inherited his gate immunity — a static object whose mask follows the person IS the strobe), and the round-66 toggle budget then zeroed the strobing pixels for the whole window, unmasking HIM as he walked through ("the w is even sitting on top of me" — a guard that erases the subject is worse than the flicker it guards). v9, from the two measured model facts (people ~1.0, furniture hovers near threshold): a component is subject only with an emphatic core (`SEG_STRONG` 0.75, `SEG_SEED_PX` 12); a window-level FURNITURE ZONE (presence ≥ 0.35, mean conf < 0.75) carves weak pixels out of kept components unless emphatic THIS frame — so a merged person+chair blob keeps the person and sheds the chair, every frame. The motion gates, standing-person exception and toggle budget all RETIRE (a standing person is kept by emphasis, not exception; the budget's only real trigger was the furniture flip-flop the zone now removes at cause). `test_matte_seg.py` pins the crossing regression with the fake model now faithful to the real one's habits.
  - **The filmstrip decoded the ORIGINAL when the proxy didn't exist yet** — the round-61 OOM class from the MAIN strip's side. Job 1619 (a 4.35 GB 4K HEVC original) killed the dispatcher three times during 300's index and took index attempt 1 down with it — that is the whole "everything was slow and the player wouldn't load" hour. Now: original-only + an ACTIVE index job → honest `available:false` (the studio re-asks the moment the proxy lands, because the asset-set signature changes); original-only + NO active index → `build_by_seek` capped at `ORIGINAL_SEEK_TILES` (60).
  - **The index greeting is idempotent against the CHAT** (job 1618 greeted twice, 3.5 min apart: reaped mid-run after the OOM death above, re-claimed, re-ran `_finish_setup` with no reindex flag). Keyed on the ASSET id in `meta.index_greet`: check-first, then `INSERT … ON CONFLICT` against the partial unique index `idx_chat_messages_index_greet` (migrations/013, **applied to prod Jul 31 2026**) so two live workers in a deploy window resolve to one message. A replacement upload is a new asset row and greets normally; retries, self-heals, version bumps and the original-lands re-index all stay quiet.
  - **Re-uploading known bytes transfers nothing** (`POST /projects/<id>/uploads/dedup`). The owner re-uploaded the same 4.35 GB clip twice in one afternoon and waited ~12 minutes each time — and the round-47 music customer re-uploaded one file four times. The client hashes first+last 8 MB (`DEDUP_CHUNK`, must mirror `_DEDUP_CHUNK` server-side) before touching anything; the server matches same-user originals on (bytes, quickhash) — verifying via two ranged GETs, cached in `meta.quickhash`; NOTE a proxy-first original carries the PROXY's sha while pending, so `upload_state` is checked, not just sha — then creates the asset `upload_state=pending` and the INDEX JOB copies server-side (`worker/storage.copy_object` is now boto3 managed transfer → multipart past 5 GB) and flips `asset_upload_ready`. Every failure answers `{dedup:false}` → normal upload: the path may only ever make an upload FASTER. The index then cache-hits on the identical sha, so the second project is editable in roughly the copy + cache-copy time.
  - **Studio: upload state is project-scoped.** The in-flight upload closures wrote page-global state with no project guard, so switching projects mid-upload carried "Uploading your video" (and the composer lock) into projects where nothing was uploading — openProject's reset was undone by the next progress tick. Every async setter in handleFile / attach / timeline-drop / picker now checks `projectIdRef.current === pid` (SETTING is scoped; clearing stays global-safe), `bgUpload` is keyed by pid and its banner renders only in its own project, and a timeline-drop that finishes after a project switch SKIPS its splice instead of editing whichever timeline is now open. Also: **the player mounts the moment a playable proxy exists** even while analysis runs (`prepping && !(playerUrl && !uploading)`) — the full-screen prep spinner used to sit over a perfectly playable proxy for the entire index; the composer's prep bar still carries the progress. **The "Viewing an earlier edit — N newer messages hidden" divider is gone and stepping back hides NOTHING** — the stepper moves the picture, the chat is history and stays (what a new message continues from is `base_version`, backend behaviour, unchanged). **The too-slow proxy abandon now reports `slower_than_upload` + measured rates** (elapsed/frac/projection/budget) — `conversion.cancel()` makes mediabunny's `execute()` reject with its own "Output has been canceled.", which escaped past the tooSlow tag and spent a day in prod beacons reading as a transcode crash.
  - The 14:53–15:00 worker deaths that afternoon were the round-67 DEPLOY killing an in-flight agent turn (Render restarts the dispatcher on push; the turn's llm_calls stop at 14:56:53, three minutes after the push landed) — the round-67c safety net covered it correctly. Deploying while a turn runs costs that turn; there is no code fix short of draining, which a minutes-long turn cannot wait for.
  - Needs the usual **`gcloud run deploy valmera-executor --source worker/ --region us-central1`** from a clean `main` — the matte job runs ON the executor, and a stale one would build v8 masks under v9 fingerprints (the round-60 false-claim class, again).
