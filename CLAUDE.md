...
- **`DEEPGRAM_API_KEY`** (worker) — the transcriber. Deepgram nova-3 is the default whenever this is set; faster-whisper is the automatic fallback and records a warning on the index when it runs. Proven in prod: on a real 19.3-min video, transcription went from **1742s (whisper medium) to 1.53s**, taking the index from 47 min to 16.5 min, and found 45 words whisper missed. `TRANSCRIBER=whisper|deepgram` forces either side. **Switching providers changes index output, so bump `PIPELINE_VERSION` (env) on BOTH services together** — they must match or the backend re-enqueues forever.
