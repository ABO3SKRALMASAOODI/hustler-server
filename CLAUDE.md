...
- **Renders pin their own clock (round 52).** Both render paths now pass `-fps_mode cfr -r {fps}`, so a rendered file's length comes from the filtergraph's TIMESTAMPS and not from whatever frame rate the last filter link advertises. Two finals failed `_verify_render` as the wrong length on Jul 27 (102.40s for a 93.50s edit; 47.11s for 45.38s) while their PREVIEWS were exact, and one user pressed Download **ten times**. Be precise about what is known: both stored EDLs render to the exact expected length against synthetic CFR, VFR and short-picture-track sources, so the specific property of those two originals was never reproduced; what IS established is the asymmetry that a preview renders from our own proxy (which `media._encode_proxy` pins for anything it detected as VFR) and a final from the customer's unpinned original. `renderer._stream_report` now logs per-stream durations, rates and frame counts on any mismatch — the datum the investigation did not have.
