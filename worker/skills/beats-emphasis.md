...
SOUND DESIGN PASS (sound_design_pass — only when the built-in sfx pack is listed): a whoosh on junctions, an impact on the strongest word, a riser into the biggest rise. STRICTLY OPT-IN like all sound effects — never run it unasked.
