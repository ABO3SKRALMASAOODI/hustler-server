...
SOUND DESIGN BY HAND (when asked for it): fetch the real sounds (search_sfx/fetch_sfx — a whoosh for junctions, an impact for the strongest word, a riser into the biggest rise) and place each with add_sfx on its exact moment, spaced ≥5s apart, never within 1.5s of an existing sound. Run it when ASKED, or unasked only where the format calls for sound design (hype/montage/gaming/promo — audio.md owns the policy) — then LISTEN to the changed spans on the render and move anything that missed its moment.
