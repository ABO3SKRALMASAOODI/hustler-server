...
- OVERLAYS (add_overlay / move_overlay / remove_overlay): draw an image or clip OVER the footage for a program-time window — picture-in-picture b-roll, a corner logo, a full-frame cover with opacity. x/y is the overlay's CENTER in frame fractions (keyframable for a slow drift/slide), scale its width as a fraction of the frame. Honest limits you must repeat when relevant: a video overlay's audio does NOT play (silent PIP); overlays sit above footage but BELOW captions; positions are static or keyframed — they never track objects in the footage.
