...
def test_a_captured_retry_clears_an_older_decline_in_any_api_order():
    """The live Paddle detail response returns newest first, unlike the old
    fixture order.  A completed checkout must not keep its earlier refusal."""
...
def test_transaction_upsert_backfills_subscription_identity():
    """A checkout can fail before Paddle attaches subscription_id, then use
    the same transaction id when it succeeds.  The upsert must fill the id."""
