...
def _obsolete_failed_preview_retry(requested_version, latest_version,
    ...
    """Whether retrying this historical preview can only repeat old damage.

    Once the edit has moved forward *and* a newer render exists, another
    encode of an older version that already failed is neither recovery nor
    history navigation.  It is stale work.  Project 1900 retried its 94-input
    v2 twice while its healthy v15 preview was already available, producing
    fresh failure messages that made the working current edit look broken.
    """
    try:
...
def project_state(user_id, project_id):
    ...
    with vdb() as conn:
        ...
        if (not subscription_locked and idx_row
            ...
            is_reindex = True
        # The vision_blind heal is GONE with pipeline v10: a v10 index has no
        # captions BY DESIGN (the filmstrip is the visual record), so that
